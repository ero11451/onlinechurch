...
export class ChatViewPage implements OnInit {
  ...
  constructor(
    ...
    chatsService.getChatRef(this.uid, this.interlocutorUID).then((chatRefPath : string) => {
      ...
      this.db.list(chatRefPath).valueChanges().subscribe((chats :  Chat[]) => { 
        console.log('chats', chats);  this.chats = chats 
      }) ;

    }); 
    this.db.object(`/users/${this.uid}`).valueChanges().subscribe((user : User) => { 
      console.log('loged user', user); this.uidData = user });
    this.db.object(`/users/${this.interlocutorUID}`).valueChanges().subscribe((user : User) => { 
      console.log('interlo', user);  this.interlocutorUIDData = user });
  }
